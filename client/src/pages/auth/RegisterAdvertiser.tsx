...
import { advertiserRegistrationSchema, AdvertiserRegistrationFormData } from '@/lib/validation';
import { 
  passwordStrength, 
  RateLimitTracker, 
  CSRFManager,
...
export default function RegisterAdvertiser() {
  ...
  const form = useForm<AdvertiserRegistrationFormData>({
  ...
  const handleSubmit = async (data: AdvertiserRegistrationFormData) => {
    ...
    try {
      ...
      const registrationData = {
        ...
        company: sanitizeInput.cleanString(data.company),
      ...
      toast({
        ...
        description: result.message || 'Ваша заявка рекламодателя отправлена на рассмотрение. С вами свяжется менеджер в течение 24 часов.',
      });

      // Redirect to login page after a short delay
      setTimeout(() => {
        setLocation('/login/advertiser');
      }, 2000);
