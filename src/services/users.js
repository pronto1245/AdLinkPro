...
export async function findUserByEmail(email) {
    try {
        const { rows } = await getPool().query(`SELECT id,email,username,role,password_hash,created_at,updated_at,
...
export async function createUser(userData) {
    ...
    try {
        ...
        const { rows } = await getPool().query(`INSERT INTO users (username, email, password, role, first_name, phone, company, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
       RETURNING id, username, email, role, first_name, phone, company, created_at, updated_at, 
                 password as password_hash,
                 COALESCE(two_factor_enabled, false) AS two_factor_enabled`, [finalUsername, email.toLowerCase(), passwordHash, role.toLowerCase(), name, phone, company]);
