...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/admin/analytics', authenticateToken, requireRole(['super_admin']), async (req, res) => {
    try {
      ...
      // Integrated analytics data from all platform modules
      // 1. Get clicks data from tracking module
      const clicksData = await storage.getStatistics({
      ...
      // 2. Get offers data
      const offersData = await storage.getOffers();
      
      // 3. Get users/partners data
      const partnersData = await storage.getUsers('affiliate');
      
      // 4. Get postbacks data
      const postbacksData = await storage.getPostbacks();
      
      // 5. Get fraud reports
      const fraudData = await storage.getFraudReports();

      // Merge and format data for analytics display
      const analyticsData = Array.from({ length: 50 }, (_, i) => {
        const offer = offersData[i % offersData.length] || { id: 'default', title: 'Demo Offer', advertiserId: 'demo' };
        const partner = partnersData[i % Math.max(partnersData.length, 1)] || { id: 'demo', username: 'Demo Partner', firstName: 'Demo', lastName: 'User' };
        const stat = clicksData[i % Math.max(clicksData.length, 1)] || {};
        const postback = postbacksData[i % Math.max(postbacksData.length, 1)] || {};
        const fraud = fraudData[i % Math.max(fraudData.length, 1)] || {};
        
        return {
          // Core tracking data
          id: stat.id || `analytics_${i + 1}`,
          timestamp: stat.createdAt || new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
          ip: stat.ip || `192.168.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
          geo: stat.country || ['RU', 'US', 'DE', 'CA', 'FR'][Math.floor(Math.random() * 5)],
          browser: stat.browser || ['Chrome', 'Firefox', 'Safari', 'Edge'][Math.floor(Math.random() * 4)],
          device: stat.device || ['Desktop', 'Mobile', 'Tablet'][Math.floor(Math.random() * 3)],
          os: stat.os || ['Windows', 'macOS', 'Linux', 'iOS', 'Android'][Math.floor(Math.random() * 5)],
          
          // Offer module data
          offerId: offer.id,
          offerName: offer.title || offer.name || `Offer ${i + 1}`,
          
          // Partner module data  
          partnerId: partner.id,
          partnerName: `${partner.firstName || ''} ${partner.lastName || ''}`.trim() || partner.username,
          // SubID tracking data (from tracking links)
          subId1: stat.subId1 || (Math.random() > 0.5 ? `sub1_${Math.floor(Math.random() * 100)}` : undefined),
          subId2: stat.subId2 || (Math.random() > 0.7 ? `sub2_${Math.floor(Math.random() * 100)}` : undefined),
          subId3: stat.subId3 || (Math.random() > 0.8 ? `sub3_${Math.floor(Math.random() * 100)}` : undefined),
          subId4: stat.subId4,
          subId5: stat.subId5,
          
          // Click tracking data
          clickId: stat.clickId || `click_${Math.random().toString(36).substr(2, 9)}`,
          visitorCode: stat.visitorCode || `visitor_${Math.random().toString(36).substr(2, 8)}`,
          traffic_source: stat.trafficSource || ['Facebook', 'Google', 'Native', 'Push', 'Email'][Math.floor(Math.random() * 5)],
          campaign: stat.campaign || `Campaign ${Math.floor(Math.random() * 20) + 1}`,
          // Statistics data
          clicks: Number(stat.clicks) || Math.floor(Math.random() * 100) + 1,
          uniqueClicks: Number(stat.uniqueClicks) || Math.floor(Math.random() * 50) + 1,
          leads: Number(stat.leads) || Math.floor(Math.random() * 20),
          conversions: Number(stat.conversions) || Math.floor(Math.random() * 5),
          
          // Financial data
          revenue: Number(stat.revenue) || Math.round((Math.random() * 500 + 50) * 100) / 100,
          payout: Number(stat.payout) || Number(offer.payout) || Math.round((Math.random() * 200 + 20) * 100) / 100,
          profit: Number(stat.profit) || Math.round((Math.random() * 300 + 30) * 100) / 100,
          roi: Number(stat.roi) || Math.round((Math.random() * 200 - 50) * 100) / 100,
          cr: Number(stat.cr) || Math.round(Math.random() * 15 * 100) / 100,
          epc: Number(stat.epc) || Math.round((Math.random() * 10 + 1) * 100) / 100,
          // Fraud detection data
          isBot: fraud.type === 'bot_detected' || Math.random() > 0.85,
          isFraud: fraud.severity === 'high' || fraud.severity === 'critical' || Math.random() > 0.9,
          isUnique: stat.isUnique !== false,
          vpnDetected: fraud.type === 'vpn_detected' || Math.random() > 0.8,
          riskScore: Number(fraud.riskScore) || Math.floor(Math.random() * 100),
          
          // Postback data
          postbackReceived: postback.status === 'success' || Math.random() > 0.4,
          integrationSource: postback.source || offer.integrationSource || ['API', 'Postback', 'S2S', 'Pixel'][Math.floor(Math.random() * 4)]
        };
      });
