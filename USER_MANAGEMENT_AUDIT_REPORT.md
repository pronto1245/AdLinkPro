...
### Пользователи с разными статусами:
- **admin** - super_admin, active, pending KYC, без страны
- **superadmin** - super_admin, active, approved KYC, RU
- **advertiser1** - advertiser, active, approved KYC, US
- **test_affiliate** - affiliate, active, pending KYC, DE
- **test_affiliate2** - affiliate, BLOCKED, rejected KYC (для тестирования фильтров)
- **staff1** - staff, active, pending KYC, GB

### Покрытие тестирования:
✅ **Роли**: все 4 роли представлены
✅ **Статусы**: активные и заблокированные
✅ **KYC**: pending, approved, rejected
✅ **География**: RU, US, DE, GB, без страны
✅ **Даты**: разные даты регистрации

## ПРОВЕРКА ФУНКЦИОНАЛЬНОСТИ

### ✅ Отображение данных работает корректно:
- Все поля подтягиваются из таблицы users
- Цветовое кодирование ролей и статусов
- Иконки для разных ролей
- Фрод-риск индикаторы

### ✅ Фильтрация работает:
- Поиск по username, email, firstName, lastName  
- Фильтр по ролям (all, super_admin, advertiser, affiliate, staff)
- Клиентская фильтрация (без вызова API)

### ✅ Кнопки и действия:
- **Создание**: форма открывается, валидация работает
- **Редактирование**: кнопка Edit с корректным data-testid
- **Удаление**: DELETE API вызов с подтверждением  
- **Фрод-анализ**: навигация на страницу фрода
- **Блокировка**: флаг подозрительности

### ✅ Безопасность:
- Доступ только для super_admin (requireRole middleware)
- Пароли исключены из API ответов
- JWT авторизация на всех endpoints

### ⚠️ Ограничения:
- Серверная фильтрация не используется (только клиентская)
- Нет пагинации (выводятся все пользователи)
- Сортировка по столбцам не реализована
- Массовые операции недоступны в UI